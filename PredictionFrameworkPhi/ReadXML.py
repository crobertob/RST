...
def record_count(db, tables):
    cursor = db.cursor()
    cursor.execute("SELECT COUNT(*) FROM " + tables[0])
...
def connect(filename):
    ...
    if create:
        create_db_struct(db)
        ''' Import the database structure from tables.xml file in same 
        directory; store the structure in the database and create the required 
